"use client";

import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { useMemo, useState } from "react";

import { useAuth } from "@/components/auth/auth-context";
import { IncomeForm, IncomeFormValues } from "@/components/forms/IncomeForm";
import { TransactionRow, TransactionTable } from "@/components/transactions/TransactionTable";
import {
  Ingreso,
  createIngreso,
  deleteIngreso,
  fetchIngresos,
  updateIngreso
} from "@/lib/api";

export default function IngresosPage() {
  const { accessToken } = useAuth();
  const queryClient = useQueryClient();
  const [editing, setEditing] = useState<Ingreso | null>(null);
  const [feedback, setFeedback] = useState<string | null>(null);
  const [formKey, setFormKey] = useState(0);

  const { data: ingresos = [], isLoading } = useQuery({
    queryKey: ["ingresos", accessToken],
    queryFn: () => fetchIngresos(accessToken ?? ""),
    enabled: Boolean(accessToken)
  });

  const invalidateData = () => {
    queryClient.invalidateQueries({ queryKey: ["ingresos"] });
    queryClient.invalidateQueries({ queryKey: ["resumen"] });
  };

  const createMutation = useMutation({
    mutationFn: (values: IncomeFormValues) =>
      createIngreso(accessToken ?? "", {
        monto: values.monto,
        fecha: values.fecha,
        tipo: values.tipo,
        observacion: values.observacion?.trim() || null
      }),
    onSuccess: () => {
      setFeedback("Ingreso registrado correctamente.");
      setFormKey((key) => key + 1);
      invalidateData();
    }
  });

  const updateMutation = useMutation({
    mutationFn: (values: IncomeFormValues) =>
      updateIngreso(accessToken ?? "", editing!.id, {
        monto: values.monto,
        fecha: values.fecha,
        tipo: values.tipo,
        observacion: values.observacion?.trim() || null
      }),
    onSuccess: () => {
      setFeedback("Ingreso actualizado correctamente.");
      setEditing(null);
      setFormKey((key) => key + 1);
      invalidateData();
    }
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteIngreso(accessToken ?? "", id),
    onSuccess: () => {
      setFeedback("Ingreso eliminado correctamente.");
      invalidateData();
    }
  });

  const rows: TransactionRow[] = useMemo(
    () =>
      ingresos.map((ingreso) => ({
        id: ingreso.id,
        fecha: new Date(ingreso.fecha).toLocaleDateString("es-CL"),
        concepto: ingreso.observacion || "Ingreso registrado",
        categoria: ingreso.tipo === "fijo" ? "Ingreso fijo" : "Ingreso eventual",
        tipo: ingreso.tipo === "fijo" ? "Fijo" : "Eventual",
        monto: ingreso.monto
      })),
    [ingresos]
  );

  const defaultValues: Partial<IncomeFormValues> | undefined = useMemo(() => {
    if (!editing) return undefined;
    return {
      monto: editing.monto,
      fecha: editing.fecha,
      tipo: editing.tipo as "fijo" | "eventual",
      observacion: editing.observacion ?? undefined
    };
  }, [editing]);

  const handleSubmit = async (values: IncomeFormValues) => {
    setFeedback(null);
    if (editing) {
      await updateMutation.mutateAsync(values);
    } else {
      await createMutation.mutateAsync(values);
    }
  };

  const handleEdit = (row: TransactionRow) => {
    const ingreso = ingresos.find((item) => item.id === row.id);
    if (ingreso) {
      setEditing(ingreso);
      setFeedback(null);
      setFormKey((key) => key + 1);
    }
  };

  const handleDelete = async (row: TransactionRow) => {
    if (!window.confirm("¿Deseas eliminar este ingreso?")) return;
    setFeedback(null);
    await deleteMutation.mutateAsync(row.id);
  };

  const activeMutationError = createMutation.error || updateMutation.error || deleteMutation.error;
  const isSubmitting = createMutation.isPending || updateMutation.isPending;

  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,400px)_1fr]">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <header className="mb-4">
          <h1 className="text-xl font-semibold text-slate-900">Registrar un ingreso</h1>
          <p className="text-sm text-slate-500">
            Añade los ingresos para mantener actualizado tu flujo de caja mensual.
          </p>
        </header>

        {(createMutation.isSuccess || updateMutation.isSuccess || deleteMutation.isSuccess) && feedback && (
          <div className="mb-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
            {feedback}
          </div>
        )}

        {activeMutationError && (
          <div className="mb-4 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-600">
            {activeMutationError instanceof Error
              ? activeMutationError.message
              : "No se pudo completar la acción. Intenta nuevamente."}
          </div>
        )}

        <IncomeForm
          key={`${formKey}-${editing?.id ?? "nuevo"}`}
          defaultValues={defaultValues}
          onSubmit={handleSubmit}
          onCancel={editing ? () => setEditing(null) : undefined}
          submitLabel={editing ? "Actualizar ingreso" : "Guardar ingreso"}
          isSubmitting={isSubmitting}
        />
      </section>

      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <header className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Ingresos del mes</h2>
            <p className="text-sm text-slate-500">
              Mantén el detalle de tus ingresos para obtener reportes más precisos.
            </p>
          </div>
          <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
            {ingresos.length} registros
          </span>
        </header>

        {isLoading && ingresos.length === 0 ? (
          <div className="h-48 animate-pulse rounded-2xl bg-gradient-to-br from-slate-200 via-slate-100 to-slate-200" />
        ) : (
          <TransactionTable
            items={rows}
            emptyMessage="Aún no registras ingresos en el sistema."
            onEdit={handleEdit}
            onDelete={handleDelete}
          />
        )}
      </section>
    </div>
  );
}
